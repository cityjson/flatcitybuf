...
// --------------------------------------------------------- appearance ---
//
// Both decoders mirror geom_decoder.rs branch for branch. Where the
// reference's shape is arguably wrong, the test says so and pins the
// reference behaviour anyway: the conformance corpus is generated by it.
...
TEST_CASE("a solid of exactly one shell keeps the solid level") {
    // solids == [1] fails the `solids[0] > 1` test and falls into the
    // MultiSolid branch, so a Solid that happens to have a single shell
    // comes back one level deeper than the same geometry written as [2].
    // Arguably wrong -- the reference does it, so the corpus contains it.
...
TEST_CASE("one surface of several rings is a MultiLineString") {
