...
///
/// Mirrors `to_cj_address` (deserializer.rs:195-216): each member is emitted
/// as a STRING iff its value is non-empty, INDEPENDENTLY of the others, and
/// the address is omitted only when every member is empty. Note the key is
/// `postcode` (not `postalCode`) and `thoroughfareNumber` is a string, not a
/// number -- cjseq2 0.2.0-alpha.1 settled this address model, pinned by
/// fcb_core's postcode/postalCode normalisation.
