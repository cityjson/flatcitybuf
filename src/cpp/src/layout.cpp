...
bool check_magic_bytes(bytes_view b) {
    if (b.size() < kMagicBytesSize)
        return false;
    static const std::uint8_t kFcb[3] = {'f', 'c', 'b'};
    if (std::memcmp(b.data() + 0, kFcb, 3) != 0)
        return false;
    if (std::memcmp(b.data() + 4, kFcb, 3) != 0)
        return false;
...
FileLayout compute_layout(std::uint32_t header_size, std::uint64_t features_count,
    ...
    l.feature_begin = checked_add(l.attr_index_begin, l.attr_index_size, "feature_begin");
    return l;
}
...
void validate_layout_against_size(const FileLayout& l, std::uint64_t total_size) {
    if (l.feature_begin > total_size) {
        throw Error(
            ...
            "sections extend past end of file: feature_begin=" + std::to_string(l.feature_begin) +
