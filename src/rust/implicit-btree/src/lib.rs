...
// fn read_node_vec<T>(node_items: &mut Vec<Entry>, mut data: impl Read) -> Result<(), Error> {
//     node_items.clear();
//     for _ in 0..node_items.capacity() {
//         node_items.push(Entry::decode_from_reader(&mut data)?);
//     }
//     Ok(())
// }
...
//         debug!(
//             "http_stream_search - index_begin: {index_begin}, feature_begin: {feature_begin} num_items: {num_items}, branching_factor: {branching_factor}, level_bounds: {level_bounds:?}, GPS bounds:[({min_x}, {min_y}), ({max_x},{max_y})]"
//         );
...
//                         debug!(
//                             "Adding new request for new level: {children_range:?} (existing queue tail: {tail:?})"
//                         );
//                         queue.push_back(children_range);
//                         continue;
//                     }
...
//                         debug!(
//                             "Adding new request for: {children_range:?} rather than merging with distant NodeRange: {tail:?} (would waste {wasted_bytes} bytes)"
//                         );
//                         queue.push_back(children_range);
//                         continue;
//                     }
...
//                     debug!(
//                         "Extending existing request {tail:?} with nearby children: {:?} (wastes {wasted_bytes} bytes)",
