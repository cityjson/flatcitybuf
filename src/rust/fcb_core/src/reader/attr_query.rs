...
impl<R: Read + Seek> FcbReader<R> {
    pub fn select_attr_query(
        ...
        let result = multi_index.query(query);
...
impl<R: Read> FcbReader<R> {
    pub fn select_attr_query_seq(
        ...
        // query: Vec<(field_name, operator, value)>
        let header = self.buffer.header();
        let Some(attr_index_info) = header.attribute_index() else {
            return Err(anyhow!("attribute index not found"));
        };

        if let Some(columns) = header.columns() {
            let rtree_offset = self.rtree_index_size();
            io::copy(&mut (&mut self.reader).take(rtree_offset), &mut io::sink())?;
            let matching_columns: Vec<_> = columns
                .iter()
                .filter(|attr| query.iter().any(|(name, _, _)| attr.name() == name))
                .collect();

            let attr_index_info = attr_index_info
                .iter()
                .map(|attr_index| {
                    let index = attr_index.index();
                    let length = attr_index.length();
                    (index, length)
                })
                .collect::<Vec<_>>();

            let mut multi_index = MultiIndex::new();
            for col in matching_columns.iter() {
                if let Some((_, length)) = attr_index_info.iter().find(|ai| ai.0 == col.index()) {
                    let mut buffer = vec![0; *length as usize];
                    match col.type_() {
                        ColumnType::Int => {
                            self.reader.read_exact(&mut buffer)?;
        ...
        Err(anyhow!("No matching columns found"))
    }
}
