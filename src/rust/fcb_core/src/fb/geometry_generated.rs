...
pub struct TransformationMatrix(pub [u8; 80]);
...
impl<'b> flatbuffers::Push for TransformationMatrix {
    ...
    #[inline]
    unsafe fn push(&self, dst: &mut [u8], _written_len: usize) {
        let src = ::core::slice::from_raw_parts(self as *const TransformationMatrix as *const u8, Self::size());
...
impl<'a> GeometryInstance<'a> {
  ...
  pub fn create<'bldr: 'args, 'args: 'mut_bldr, 'mut_bldr, A: flatbuffers::Allocator + 'bldr>(
    ...
    if let Some(x) = args.transformation { builder.add_transformation(x); }
    builder.finish()
  }
  ...
  pub fn transformation(&self) -> Option<&'a TransformationMatrix> {
    ...
    // Created from valid Table for this object
    // which contains a valid value in this slot
    unsafe { self._tab.get::<TransformationMatrix>(GeometryInstance::VT_TRANSFORMATION, None)}
  }
  #[inline]
  pub fn template(&self) -> u32 {
    ...
    // Created from valid Table for this object
    // which contains a valid value in this slot
    unsafe { self._tab.get::<u32>(GeometryInstance::VT_TEMPLATE, Some(0)).unwrap()}
  }
  #[inline]
  pub fn boundaries(&self) -> Option<flatbuffers::Vector<'a, u32>> {
    // Safety:
    // Created from valid Table for this object
    // which contains a valid value in this slot
    unsafe { self._tab.get::<flatbuffers::ForwardsUOffset<flatbuffers::Vector<'a, u32>>>(GeometryInstance::VT_BOUNDARIES, None)}
  }
}
...
impl<'a: 'b, 'b, A: flatbuffers::Allocator + 'a> GeometryInstanceBuilder<'a, 'b, A> {
  ...
  pub fn add_transformation(&mut self, transformation: &TransformationMatrix) {
    self.fbb_.push_slot_always::<&TransformationMatrix>(GeometryInstance::VT_TRANSFORMATION, transformation);
  }
  #[inline]
  pub fn add_template(&mut self, template: u32) {
    self.fbb_.push_slot::<u32>(GeometryInstance::VT_TEMPLATE, template, 0);
  }
  #[inline]
  pub fn add_boundaries(&mut self, boundaries: flatbuffers::WIPOffset<flatbuffers::Vector<'b , u32>>) {
    self.fbb_.push_slot_always::<flatbuffers::WIPOffset<_>>(GeometryInstance::VT_BOUNDARIES, boundaries);
  }
  #[inline]
  pub fn new(_fbb: &'b mut flatbuffers::FlatBufferBuilder<'a, A>) -> GeometryInstanceBuilder<'a, 'b, A> {
    let start = _fbb.start_table();
