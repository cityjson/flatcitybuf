...
use fcb_core::{
    attribute::AttributeSchemaMethods, read_cityjson_from_reader, CJType, CJTypeKind, FcbReader,
};
...
struct Args {
    /// Directory containing pairs of .fcb and .city.jsonl files
    #[arg(short, long)]
    dir: PathBuf,
...
fn analyze_fcb(path: &Path) -> Result<(u64, usize, usize, usize)> {
    ...
    let attribute_count = header.columns().iter().len();
...
/// Analyze CityJSONSeq file to determine file size
fn analyze_jsonl(path: &Path) -> Result<u64> {
...
fn main() -> Result<()> {
    ...
    for (fcb_path, jsonl_path, city_name) in pairs {
        ...
        // Analyze FCB file
        let (feature_count, city_object_count, vertex_count, attribute_count) =
        ...
        let jsonl_file_size = match analyze_jsonl(&jsonl_path) {
            Ok(size) => size,
            Err(e) => {
...
fn output_csv(stats: &[FileStats]) -> Result<()> {
    println!("city,fcb_size,jsonl_size,compression,features,city_objects,vertices,attributes");
