...
fn read_fcb_with_attr_index_seekable(path: PathBuf) -> Result<()> {
...
fn main() {
    // Get the path to the fcb_core crate directory
    let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let input_file_path = manifest_dir.join("benchmark_data/attribute/3dbag_partial.fcb");
    read_fcb_with_attr_index_seekable(input_file_path).unwrap();
}
