...
def _poc_address(hdr: Any) -> Optional[Dict[str, Any]]:
    ...
    Mirrors deserializer.rs's to_cj_address (deserializer.rs:195-216): each
    member is emitted as a STRING iff its value is non-empty, INDEPENDENTLY of
    the others, and the address is omitted only when every member is empty.
    Note the key is `postcode` (not `postalCode`) and `thoroughfareNumber` is a
    string, not a number -- cjseq2 0.2.0-alpha.1 settled this address model."""
    members: Dict[str, Any] = {}

    def insert(key: str, raw: Any) -> None:
        if raw is None:
            return
        value = _decode_str(raw)
        if value:
            members[key] = value

    insert("thoroughfareNumber", hdr.PocAddressThoroughfareNumber())
    insert("thoroughfareName", hdr.PocAddressThoroughfareName())
    insert("locality", hdr.PocAddressLocality())
    insert("postcode", hdr.PocAddressPostcode())
    insert("country", hdr.PocAddressCountry())
