...
- **Node ≥ 20** for development and tests (24.11.1 is what is installed).
...
- **Pinned versions:** `vite@^8.1.5`, `vitest@5.0.0-beta.6`, `@vitest/browser@5.0.0-beta.6`, `flatbuffers@^25.9.23` (matching the locally installed `flatc 25.9.23`).
...
src/ts/
  ...
  src/
    ...
    io/
      range-reader.ts     # RangeReader, BufferedRangeReader        <- Task 5
      bytes.ts            # ArrayBuffer / Uint8Array                <- Task 5
...
Create `.github/workflows/ci-ts.yml` running those three on `ubuntu-latest` with Node 20 and 24, with **no Rust toolchain**. Leave the wasm jobs alone until Task 18.
...
Tracked: `conformance/inputs/*.city.jsonl` and `conformance/*.expected.jsonl` (15 files). **`.fcb` files are gitignored** and regenerated by the script — do not try to commit them. An untracked `conformance/` directory may already exist locally from a worktree run; `git mv` over it, or delete it first.

- [ ] **Step 1: Move and repoint**

```bash
rm -rf conformance   # untracked leftover, if present
...
```bash
git add -A conformance src/cpp scripts/gen_conformance.sh
...
describe('rtreeIndexSize', () => {
  it('matches the reference formula', () => {
    ...
    expect(rtreeIndexSize(1, 16)).toBe(40)
    expect(rtreeIndexSize(16, 16)).toBe((16 + 1) * 40)
    expect(rtreeIndexSize(17, 16)).toBe((17 + 2 + 1) * 40)
  })

  it('clamps the node size to [2, 65535]', () => {
    expect(rtreeIndexSize(4, 0)).toBe(rtreeIndexSize(4, 2))
    expect(rtreeIndexSize(4, 99999)).toBe(rtreeIndexSize(4, 65535))
  })
})
...
describe('computeLayout', () => {
  it('places sections back to back with no padding', () => {
    ...
    expect(l.rtreeSize).toBe(40)
    expect(l.attrIndexBegin).toBe(l.headerLen + 40)
    expect(l.featureBegin).toBe(l.headerLen + 40)
  })
...
**Files:**
- Create: `src/ts/src/io/range-reader.ts`, `src/ts/src/io/bytes.ts`, `src/ts/test/fixtures/counting-reader.ts`, `src/ts/test/range-reader.test.ts`
...
describe('readHeader', () => {
  it('reads version, counts and transform from small.fcb', async () => {
    const { info } = await readHeader(open('small.fcb'))
    expect(info.version).toBe('2.0')
    expect(info.featuresCount).toBeGreaterThan(0)
    expect(info.scale).toHaveLength(3)
    expect(info.translate).toHaveLength(3)
    expect(info.scale.every(Number.isFinite)).toBe(true)
  })

  it('reads a geographical extent of exactly six finite numbers', async () => {
    const { info } = await readHeader(open('small.fcb'))
    expect(info.geographicalExtent).toHaveLength(6)
    expect(info.geographicalExtent!.every(Number.isFinite)).toBe(true)
  })
  ...
  it('treats featuresCount 0 as UNKNOWN, and says so in the type docs', async () => {
    // Pinned so nobody "optimizes" a scan into an early return on 0.
    const { info } = await readHeader(open('small.fcb'))
...
describe('AttributeIndex struct', () => {
  it('is 16 bytes with padding after each ushort, not 12', async () => {
    // Field order in header.fbs forces 2 bytes of padding after each ushort.
    // Reading it as 12 silently walks off into the next entry.
    const { info } = await readHeader(open('duplicate_keys.fcb'))
    for (const ai of info.attributeIndices) {
      expect(ai.branchingFactor).toBeGreaterThanOrEqual(2)
      expect(ai.branchingFactor).toBeLessThanOrEqual(65535)
      expect(ai.length).toBeGreaterThan(0)
      expect(Number.isSafeInteger(ai.begin)).toBe(true)
    }
  })
...
describe('sequential scan', () => {
  ...
  it('yields durable handles that survive advancing the cursor', async () => {
    // The handle owns a copy of its bytes, so this must NOT throw or return
    // the wrong feature's data.
    const r = await open(resolve(CORPUS, 'small.fcb'))
    const held = []
    for await (const f of await r.selectAll()) held.push(f)
    expect(held[0]!.id).not.toBe(held[held.length - 1]!.id)
    expect(() => held[0]!.cityObjects()).not.toThrow()
  })

  it('throws on a re-entrant next() rather than interleaving state', async () => {
    const r = await open(resolve(CORPUS, 'small.fcb'))
    const it = (await r.selectAll())[Symbol.asyncIterator]()
    const a = it.next()
    await expect(it.next()).rejects.toThrow(/reentrant/i)
    await a
  })
})
...
- [ ] **Step 3: Implement.** Order: `attribute.ts` (pure, no I/O), then `feature/index.ts` (framing + `Feature`), then `reader.ts` (`FcbReader.fromBytes`/`fromBlob`, `selectAll`, the cursor). Framing: read the 4-byte LE prefix, validate `0 < len <= MAX_FEATURE_SIZE` and `offset + 4 + len <= size()`, read the body, **copy** into a fresh `Uint8Array`, hand it to `CityFeature.getSizePrefixedRootAsCityFeature`. Scan to EOF rather than to `featuresCount` — `0` means unknown.
...
describe('material values', () => {
  ...
  it('keeps a shared material of index 0, which is falsy in JS', () => {
...
describe.each(CASES)('conformance: %s', (name) => {
  it('matches the Rust reader line for line', async () => {
    ...
    for (const key of ['type', 'version', 'transform', 'geometry-templates']) {
      if (key in expected[0]) {
        expect((actual[0] as Record<string, unknown>)[key]).toEqual(expected[0][key])
      }
    }
...
Add a separate test for the int64 policy:

```ts
it('emits int64 attributes as numbers by default and as strings on request', async () => {
  // Pinned so the default stays JSON-serializable: JSON.stringify throws on
  // a BigInt, so a raw bigint in the emitted object would break every consumer.
  const r = await FcbReader.fromBytes(
    new Uint8Array(readFileSync(resolve(CORPUS, 'inferable_types.fcb'))))
  for await (const f of await r.selectAll()) {
    expect(() => JSON.stringify(toCityJSONFeature(f, r.header))).not.toThrow()
    expect(() => JSON.stringify(
      toCityJSONFeature(f, r.header, { int64: 'decimal-string' }))).not.toThrow()
  }
})
```
...
- Produces:
  ...
  export function searchRtree(reader: RangeReader, rtreeBegin: number, numItems: number, nodeSize: number, query: SpatialQuery): Promise<SearchResultItem[]>
  // reader.ts
...
describe('bbox search', () => {
  ...
  it('finds a single known feature by its own bbox', async () => {
    // Expected id obtained from the C++ reader as oracle -- do not hand-derive.
    const r = await FcbReader.fromBytes(bytes('small.fcb'))
    const first = (await ids(await r.selectAll()))[0]!
    const e = r.header.info.geographicalExtent!
    const hit = await ids(await r.select({
      spatial: { kind: 'bbox', value: [e[0], e[1], e[3], e[4]] },
    }))
    expect(hit).toContain(first)
  })

  it('treats pointIntersects as a degenerate bbox', async () => {
    const r = await FcbReader.fromBytes(bytes('small.fcb'))
    const e = r.header.info.geographicalExtent!
    const cx = (e[0] + e[3]) / 2, cy = (e[1] + e[4]) / 2
    const p = await ids(await r.select({ spatial: { kind: 'point', value: [cx, cy] } }))
    const b = await ids(await r.select({ spatial: { kind: 'bbox', value: [cx, cy, cx, cy] } }))
    expect(p.sort()).toEqual(b.sort())
  })
...
- Produces:
  ```ts
  export type KeyKind = 'i8' | 'u8' | 'i16' | 'u16' | 'i32' | 'u32' | 'i64' | 'u64' | 'f32' | 'f64' | 'bool' | 'datetime' | 'str50' | 'str100'
  ...
  export function decodeKey(kind: KeyKind, dv: DataView, offset: number): unknown
...
**Column type → key kind, as the writer actually emits:** `Bool→bool, Byte→u8, UByte→u8, Short→i16, UShort→u16, Int→i32, UInt→u32, Long→i64, ULong→u64, Float→f32, Double→f64, String→str50, DateTime→datetime, Json→str100, Binary→str100`. `str20` is defined upstream but never produced.
...
describe('string keys', () => {
  it('truncates at the BYTE level, splitting a UTF-8 sequence if it must', () => {
    const long = 'é'.repeat(40)                            // 80 bytes
    expect(encodeKey('str50', long)).toHaveLength(50)
  })
...
**Two things this task must get right that no Format Reference row will tell you:**

1. **Do NOT port Rust's operator lowering.** The Format Reference documents it faithfully and it is a known-live defect: `Gt`/`Lt`/`Ne` are computed as a range *minus* `find_exact`, and the subtraction operates on **feature offsets**, so a feature whose CityObjects carry both `k` and `k' > k` is deleted from its own `Gt(k)` result (`docs/upstream-findings.md:130-145`, "NOT FIXED upstream"). **Follow C++: evaluate strict-or-inclusive bounds at the leaf.** One traversal, no subtraction, no false negatives.
2. **`PAYLOAD_TAG` is a real bit in the wire data.** Test and strip it in BigInt, then convert. `Number((1n << 63n) | 12345n)` loses the 12345. Never write the tag as `1 << 63`, which in JS is `-2147483648`.

Multi-condition queries are AND-intersected sequentially with early exit on empty.

- [ ] **Step 0: Get the expected values from the oracle before writing the test**

The test below needs one real indexed value from `duplicate_keys.fcb`. Do not guess it and do not read it out of your own half-built decoder — that would make the test pass against its own bug. Print it from a reference implementation:

```bash
# The C++ reader as oracle. Add a temporary case to src/cpp/tests that opens
# conformance/duplicate_keys.fcb, prints header.info().columns[0].name and
# every distinct value of that column, run it, then revert the injection.
cd src/cpp && cmake --build build-native -j8 && ./build-native/tests/fcb_tests
```

Record the column name and a value that **more than one feature carries** — that is the case finding #5 breaks — and substitute both into the test below.
...
describe('payload tag', () => {
  ...
  it('survives Number() only when stripped in bigint first', () => {
    ...
    expect(Number(tagged)).toBe(Number(PAYLOAD_TAG))  // low bits destroyed
    expect(Number(tagged & (PAYLOAD_TAG - 1n))) .toBe(12345)
  })
})

describe('attribute queries', () => {
  it('Eq returns exactly the features holding that value', async () => {
    // Expected ids taken from the C++ reader running the same query.
    const r = await FcbReader.fromBytes(bytes('duplicate_keys.fcb'))
    const all = await ids(await r.selectAll())
    const col = r.header.info.columns[0]!
    const hit = await ids(await r.select({
      where: [{ field: col.name, operator: 'Eq', value: /* oracle value */ 0 }],
    }))
    expect(hit.every((id) => all.includes(id))).toBe(true)
  })

  it('Gt does NOT drop a feature that also holds a smaller value', async () => {
    // Upstream finding #5: Rust computes Gt as range-minus-exact over FEATURE
    // offsets, so a feature carrying both k and k' > k deletes itself. We use
    // strict leaf bounds instead, as C++ does. duplicate_keys.fcb exists
    // precisely because one feature carries several values of one attribute.
    const r = await FcbReader.fromBytes(bytes('duplicate_keys.fcb'))
    const col = r.header.info.columns[0]!
    const gt = await ids(await r.select({
      where: [{ field: col.name, operator: 'Gt', value: 0 }],
    }))
    const ge = await ids(await r.select({
      where: [{ field: col.name, operator: 'Ge', value: 0 }],
    }))
    const eq = await ids(await r.select({
      where: [{ field: col.name, operator: 'Eq', value: 0 }],
    }))
    // Every Gt hit is a Ge hit, and Ge is the union of Gt and Eq -- which is
    // exactly what the subtraction lowering breaks.
    expect(gt.every((id) => ge.includes(id))).toBe(true)
    expect(new Set(ge)).toEqual(new Set([...gt, ...eq]))
  })
  ...
  it('AND-intersects multiple conditions', async () => {
    ...
    const both = await ids(await r.select({
      where: [
        { field: col.name, operator: 'Ge', value: 0 },
        { field: col.name, operator: 'Le', value: 0 },
      ],
    }))
    const eq = await ids(await r.select({
      where: [{ field: col.name, operator: 'Eq', value: 0 }],
    }))
    expect(new Set(both)).toEqual(new Set(eq))
  })
  ...
  it('rejects Json and Binary index queries, as Rust and C++ do', async () => {
    // Deliberate divergence #2: these are FixedStringKey<100> over a blob, so
    // index hits are near-meaningless. Rejecting is honest.
    const r = await FcbReader.fromBytes(bytes('inferable_types.fcb'))
    const json = r.header.info.columns.find((c) => c.type === /* Json */ 13)
    if (json) {
      await expect(r.select({ where: [{ field: json.name, operator: 'Eq', value: '{}' }] }))
...
Replace the `/* oracle value */` placeholder with a real value obtained by running the equivalent query through `./src/cpp/build-native/tests/fcb_tests` or the Rust CLI **before** implementing — do not guess it.
...
- [ ] **Step 0: Get the colliding string pair from the oracle**

The test needs a real value from `long_strings.fcb` whose first 50 bytes match another value in the same column. Print them rather than inventing them:

```bash
python3 - <<'EOF'
import json
seen = {}
for line in open('conformance/long_strings.expected.jsonl'):
    line = line.strip()
    ...
    for co in obj.get('CityObjects', {}).values():
        for k, v in (co.get('attributes') or {}).items():
            if isinstance(v, str):
                seen.setdefault((k, v.encode('utf8')[:50]), []).append(v)
for (col, prefix), vals in seen.items():
    if len(set(vals)) > 1:
        print('COLLIDING', col, vals[:2])
EOF
```

Substitute the printed column name and full value into the test. If nothing collides, the fixture does not exercise this and you must construct one: write a tiny `.city.jsonl` with two features whose string attribute agrees in its first 50 bytes, run it through the Rust CLI, and add it to the corpus.
...
describe('string post-filtering', () => {
  it('does not return a feature whose full value merely SHARES a truncated key', async () => {
    // long_strings.fcb holds values that agree in their first 50 bytes and
    // differ after. Without post-filtering the tree returns both.
    const r = await FcbReader.fromBytes(bytes('long_strings.fcb'))
    const col = r.header.info.columns.find((c) => c.type === /* String */ 12)!
    const target = /* oracle: a full value from the file */ ''
    const hit = await ids(await r.select({
      where: [{ field: col.name, operator: 'Eq', value: target }],
    }))
    for (const id of hit) expect(id).toBeTruthy()
    expect(hit.length).toBeLessThanOrEqual(
      (await ids(await r.selectAll())).length)
  })
  ...
  it('post-filters SHORT queries too, because zero padding also collides', async () => {
    ...
    const col = r.header.info.columns.find((c) => c.type === /* String */ 12)!
    const hit = await ids(await r.select({
      where: [{ field: col.name, operator: 'Eq', value: 'a' }],
    }))
    // 'a' and 'a\0' share a key; only an exact full-value match may survive.
    expect(hit).toEqual([])
  })
  ...
  it('reports featuresCount AFTER post-filtering, not the candidate count', async () => {
    ...
    expect(cursor.featuresCount).toBe((await ids(cursor)).length)
  })
})
...
describe('composition', () => {
  it('intersects a spatial and an attribute predicate', async () => {
    ...
    const e = r.header.info.geographicalExtent!
    const col = r.header.info.columns[0]!
    const where = [{ field: col.name, operator: 'Ge' as const, value: 0 }]
    const attrOnly = await ids(await r.select({ where }))
    const both = await ids(await r.select({
      spatial: { kind: 'bbox', value: [e[0], e[1], e[3], e[4]] }, where,
    }))
    expect(new Set(both)).toEqual(new Set(attrOnly))
  })
...
describe('pointNearest', () => {
  ...
  it('returns nothing for a file with no features', async () => {
    const r = await FcbReader.fromBytes(bytes('single_feature.fcb'))
    const cursor = await r.select({ spatial: { kind: 'nearest', value: [0, 0] } })
    expect(cursor.featuresCount).toBe(1)
  })

  it('agrees with a brute-force scan over every feature centroid', async () => {
    // The oracle that does not depend on heap order: compute the nearest
    // centroid by scanning, and compare DISTANCE rather than identity so an
    // exact tie does not make the test flaky.
    const r = await FcbReader.fromBytes(bytes('small.fcb'))
    const e = r.header.info.geographicalExtent!
    const px = (e[0] + e[3]) / 2, py = (e[1] + e[4]) / 2
    const got = (await ids(await r.select({
...
Everything above runs under Node. This task adds the additive browser job so the *actual shipping target* is exercised. It is last because **no core task may depend on browser mode** — Vitest 5 browser mode is the newest tool in the stack.
...
```bash
cd src/ts && npx playwright install chromium && npx vitest run --project=browser
```
