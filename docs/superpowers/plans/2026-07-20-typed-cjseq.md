...
- **`.fcb` files are gitignored too** (`.gitignore:65`). The conformance corpus tracks only `inputs/*.city.jsonl` and `*.expected.jsonl`; the binaries are regenerated by `scripts/gen_conformance.sh`.
